...
class _ProfileEditState extends State<ProfileEdit> {
  ...
  fetchUserInfo() async {
    ...
    print(userInfoResponse?.data?[0].name);
    _nameController =
        TextEditingController(text: userInfoResponse?.data?[0].name);
    _phoneController =
        TextEditingController(text: userInfoResponse?.data?[0].phone);
    setState(() {
      _isLoading = false;
    });
  }
  ...
  onPressUpdate() async {
    ...
    var post_body =
        jsonEncode({"name": "${name}", "phone":  phone});
